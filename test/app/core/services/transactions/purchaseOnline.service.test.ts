...
describe('purchaseOnlineService', () => {
  const parameter = {
    amount: '20',
    accountUser: '1234456',
  ...
  test('should call dinHeaderMapper with correct parameters', async () => {
    await purchaseOnlineService(parameter);
  ...
  test('should call http with the correct parameters', async () => {
    ...
    expect(http).toHaveBeenCalledWith({
      url: urlResources.purchaseOnline,
  ...
  test('should return mapped response when request is successful', async () => {
    const result = await purchaseOnlineService(parameter);
